...
/// Verifies an SP1 v6 Groth16 proof against the v6.1.0 circuit.
///
/// Format differences vs v5:
///   - proof layout: [4B selector | 32B exitCode | 32B vkRoot | 32B nonce | 256B Groth16]
///   - public inputs: 5 (vkey_hash, committed_values_digest, exitCode, vkRoot, nonce)
///
/// Mirrors `SP1VerifierGroth16.sol::verifyProof` from the v6.1.0 reference contract.
/// Caller must supply the expected `vk_root` constant for the SP1 version:
///   - v6.1.0: 0x002f850ee998974d6cc00e50cd0814b098c05bfade466d28573240d057f25352
#[inline]
pub fn verify_proof_v6(
    proof: &[u8],
    sp1_public_inputs: &[u8],
    sp1_vkey_hash: &str,
    groth16_vk: &[u8],
    ...
    let exit_code: [u8; 32] = proof[4..36].try_into().map_err(|_| Error::InvalidInput)?;
    let vk_root: [u8; 32] = proof[36..68].try_into().map_err(|_| Error::InvalidInput)?;
    let nonce: [u8; 32] = proof[68..100].try_into().map_err(|_| Error::InvalidInput)?;

    if exit_code != [0u8; 32] {
        return Err(Error::InvalidExitCode);
    }
    if vk_root != *expected_vk_root {
        return Err(Error::InvalidVkRoot);
    }

    let sp1_vkey_hash_bytes = decode_sp1_vkey_hash(sp1_vkey_hash)?;
    let vkey_hash_31: [u8; 31] = sp1_vkey_hash_bytes[1..]
        .try_into()
        .map_err(|_| Error::InvalidProgramVkeyHash)?;
    let committed_digest = hash_public_inputs(sp1_public_inputs);

    let inputs_struct = load_public_inputs_v6_from_bytes(
        &vkey_hash_31,
        &committed_digest,
        &exit_code,
        &vk_root,
...
/// Like `verify_proof_raw` but takes a 5-input PublicInputs struct.
fn verify_proof_raw_v6(
    proof: &[u8],
    public_inputs: &PublicInputs<5>,
    vk: &[u8],
) -> Result<(), Error> {
    let proof = load_proof_from_bytes(proof)?;
    let vk = load_groth16_verifying_key_from_bytes(vk)?;
