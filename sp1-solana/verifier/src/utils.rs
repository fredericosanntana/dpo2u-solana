...
pub enum Error {
    ...
    #[error("SP1 v6 exit code is non-zero")]
    InvalidExitCode,
    #[error("SP1 v6 vk_root mismatch")]
    InvalidVkRoot,
}
...
/// SP1 v6 public inputs: (vkey_hash, committed_values_digest, exit_code, vk_root, nonce).
/// vkey_hash is 31 bytes left-padded to 32; all others are full 32-byte big-endian field elements.
pub(crate) fn load_public_inputs_v6_from_bytes(
    sp1_vkey_hash_31: &[u8; 31],
    committed_values_digest: &[u8; 32],
    exit_code: &[u8; 32],
    vk_root: &[u8; 32],
    nonce: &[u8; 32],
) -> Result<PublicInputs<5>, Error> {
    let mut padded_vkey = [0u8; 32];
    padded_vkey[1..].copy_from_slice(sp1_vkey_hash_31);
    Ok(PublicInputs::<5> {
        inputs: [
            padded_vkey,
